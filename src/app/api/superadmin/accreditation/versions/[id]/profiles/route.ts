...
import {
  createSuperadminVersionProfile,
  listSuperadminVersionProfiles,
...
export async function GET(
  ...
  const result = await listSuperadminVersionProfiles(id);
...
export async function POST(
  ...
  const { id } = await params;
  const result = await createSuperadminVersionProfile(id, body);
