...
export async function GET() {
  ...
  const result = await listInstitutionalDataSources(session.tenantId, session.userId, session.role);
...
export async function POST(request: Request) {
  ...
  const result = await createInstitutionalDataSource(session.tenantId, parsed.body, session.userId, session.role);
  return NextResponse.json(result, { status: result.status === "success" ? 200 : 400 });
}
