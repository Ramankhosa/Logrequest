...
import {
  ...
  listAssessmentWorkspaceDiscussionThreads,
...
export async function GET(
  ...
  const result = await listAssessmentWorkspaceDiscussionThreads(
    id,
    session.tenantId,
    session.userId,
    session.role,
...
export async function POST(
  ...
  const { id } = await params;
  const result = await createAssessmentWorkspaceDiscussionThread(
    id,
    session.tenantId,
    parsed.body,
    session.userId,
