...
import {
  createWorkspaceDvvQuery,
  listWorkspaceDvvQueries,
} from "@/lib/accreditation/workspace-reporting-service";
import {
...
export async function GET(
  ...
  const result = await listWorkspaceDvvQueries(id, session.tenantId, session.userId, session.role);
...
export async function POST(
  ...
  const { id } = await params;
  const result = await createWorkspaceDvvQuery(
    id,
    session.tenantId,
    parsedBody.body,
    session.userId,
