...
export function UserRoleUpload() {
  ...
  const handleFile = async (f: File) => {
    ...
    try {
      ...
      const res = await fetch("/api/tenant/structure/user-import", {
        method: "POST",
        body: formData,
      });
  ...
  const handleDownloadTemplate = async () => {
    try {
      const res = await fetch("/api/tenant/structure/user-import/template");
      if (!res.ok) throw new Error();
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "user-role-import-template.csv";
      a.click();
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-400">
            Upload a CSV or Excel file to bulk-import users and assign them
            roles at org units
          </p>
        </div>
        <button
            ...
            <>
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-slate-400">
                  Columns: email, first_name, last_name, employee_id,
                  unit_code, role_key
                </p>
              </div>
            </>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              Import {preview.validCount} user(s)
            </button>
            <button
              type="button"
              onClick={reset}
