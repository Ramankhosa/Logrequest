...
export function KraCategoryList() {
  ...
  const fetchCategories = useCallback(async () => {
    try {
      const res = await fetch("/api/tenant/kra-kpi/categories");
  ...
  const handleDelete = async (cat: CategoryView) => {
    if (!window.confirm(`Delete category "${cat.displayLabel}"?`)) return;
    setDeletingId(cat.id);
    try {
      const res = await fetch(`/api/tenant/kra-kpi/categories/${cat.id}`, { method: "DELETE" });
      const data = await res.json();
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold text-slate-900">KRA Categories</h3>
          <p className="mt-0.5 text-xs text-slate-400">
            Organize KRAs by categories. Global categories are visible to all tenants.
          </p>
        </div>
        {!addingNew && (
          <button
      ...
      {addingNew && (
        <CategoryForm
          mode="create"
          onDone={() => { setAddingNew(false); showFeedback("success", "Category created."); void fetchCategories(); }}
      ...
      {categories.length === 0 && !addingNew ? (
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-slate-200 py-12 text-center">
          ...
          <p className="text-xs text-slate-400">Create categories to organize your KRAs</p>
        </div>
      ) : (
          ...
          {categories.map((cat) =>
            editingId === cat.id ? (
              <CategoryForm
                ...
                mode="edit"
                initial={cat}
                onDone={() => { setEditingId(null); showFeedback("success", "Category updated."); void fetchCategories(); }}
              ...
              <div key={cat.id} className={`group flex items-center gap-3 rounded-xl border border-slate-200/80 bg-white px-4 py-3.5 transition hover:border-slate-300 ${!cat.isActive ? "opacity-50" : ""}`}>
                <div
                  ...
                  style={{ backgroundColor: cat.colorHex ? `${cat.colorHex}20` : undefined, color: cat.colorHex ?? undefined }}
                >
                  {cat.scope === "GLOBAL" ? <Globe className="h-4 w-4" /> : <Building2 className="h-4 w-4" />}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`rounded-md px-1.5 py-0.5 text-[9px] font-bold uppercase ${cat.scope === "GLOBAL" ? "bg-blue-50 text-blue-500" : "bg-slate-50 text-slate-500"}`}>{cat.scope}</span>
                  </div>
                ...
                {cat.scope === "TENANT" && (
                  <div className="flex shrink-0 items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                    <button type="button" onClick={() => setEditingId(cat.id)} className="flex h-7 w-7 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 transition hover:border-blue-200 hover:text-blue-600" title="Edit">
                      <Pencil className="h-3.5 w-3.5" />
                    </button>
                    <button type="button" onClick={() => handleDelete(cat)} disabled={deletingId === cat.id} className="flex h-7 w-7 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 transition hover:border-rose-200 hover:text-rose-600 disabled:opacity-50" title="Delete">
...
function CategoryForm({ mode, initial, onDone, onCancel }: {
  mode: "create" | "edit";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = mode === "create" ? "/api/tenant/kra-kpi/categories" : `/api/tenant/kra-kpi/categories/${initial!.id}`;
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {mode === "create" && (
          <div>
            ...
            <input type="text" placeholder="e.g. RESEARCH" value={categoryKey} onChange={(e) => setCategoryKey(e.target.value.toUpperCase())} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 font-mono text-sm uppercase outline-none placeholder:text-slate-400 focus:border-brand focus:ring-1 focus:ring-brand/30" autoFocus />
          </div>
        )}
        <div>
          <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-slate-500">Display label</label>
          <input type="text" placeholder="e.g. Research & Innovation" value={displayLabel} onChange={(e) => setDisplayLabel(e.target.value)} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none placeholder:text-slate-400 focus:border-brand focus:ring-1 focus:ring-brand/30" autoFocus={mode === "edit"} />
        </div>
        <div className="sm:col-span-2">
          <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-slate-500">Description</label>
          <input type="text" placeholder="Optional description" value={description} onChange={(e) => setDescription(e.target.value)} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none placeholder:text-slate-400 focus:border-brand focus:ring-1 focus:ring-brand/30" />
        </div>
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <input type="number" value={sortOrder} onChange={(e) => setSortOrder(Number(e.target.value))} min={0} max={9999} className="w-20 rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-sm outline-none focus:border-brand focus:ring-1 focus:ring-brand/30" />
          </div>
        </div>
      </div>
